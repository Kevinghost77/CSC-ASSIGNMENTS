...
public class AVLTree<dataType extends Comparable<? super dataType>> extends BinaryTree<dataType>
{
   ...
   public void insert ( dataType d ,dataType Statement, dataType Score )
   {
      ...
      root = insert (d, Statement, Score, root);
   }
   public BinaryTreeNode<dataType> insert ( dataType d, dataType Statement, dataType Score, BinaryTreeNode<dataType> node )
   {  
        
        
      if (node == null & inCount++ >=0){
             
         return new BinaryTreeNode<dataType> (d, Statement, Score, null, null);
          }
      if (d.compareTo (node.data) <= 0){
         
         node.left = insert (d, Statement, Score, node.left);}
      else{
         
         node.right = insert (d,Statement, Score, node.right);}
      return balance (node);
   }
   ...
   public BinaryTreeNode<dataType> find ( dataType d, BinaryTreeNode<dataType> node )
   {
      ...
      if (d.compareTo (node.data) == 0 & seCount++ >= 0) {
   ...
   public int getSearchCount(){return seCount;}
   public int getInsertCount(){return inCount;}
   public int clearSearchCount(){return seCount = 0;}
   public int clearInsertCount(){return inCount = 0;}
}
